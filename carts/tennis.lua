...
function new_game(is_demo)
	...
	for _, v in pairs(active_player_settings) do
		...
		player.power = 0
		player.lob = false
		player.pos_data = team_index == serving_team and server_data[team_index + 1][team_member_index + 1] or
			receiver_data[team_index + 1][team_member_index + 1]
...
function animate_limbs(sprite_model, anim, new_time)
	...
	for i = 1, last_frame do
		if new_time < anim.frames[i].to_time
			...
			set_pose(sprite_model, prev_frame, anim.frames[i],
				(new_time - prev_frame_time) / (anim.frames[i].to_time - prev_frame_time))
			break
		end
	...
	elseif p.mode == 1 then
		if ai_think then
			if ball.last_hit_player and ball.last_hit_player.team ~= p.team
				...
				p.move_to = { dest }
				p.ai_hit_distance = swing_dist * 0.5 + math.random() * swing_dist * 0.5 + math.random() * ai_dumbness *
					0.25
...
function update_player(p)
	...
	if p.control_type ~= PLAYER_NONE then
		...
		if v3d_length(p.vel) < 0.1 then
			...
			p.leg_anim = anims
			[math.floor((move_angle < 0 and move_angle + 1 or (move_angle >= 1 and move_angle - 1 or move_angle) * 4)) + 2]
		end
		...
		if p.swing_timer > 0 then
			...
			if ((p.swing_timer == 30 and ball.hit_count == 0) or p.swing_timer == 25) and (ball.vel.z == 0 or (ball.vel.z > 0) ~= (p.facing > 0)) then
				...
				if ball_distance <= hit_range then
					...
					direction = (dx + math.abs(ball.pos.x - p.pos.x) / (hit_range * 2.5)) * -p.swing_dir * p.move_dir *
						p.camera_side + ( -ball.pos.x / 64) * 1.2
					vert = (ball.pos.y / 18 + lerp(0.9, 1.0, math.abs(p.pos.z / 58) * 0.5 + (ball_distance / hit_range * 0.4))) *
						-1.25
					if p.lob then
	...
	for k, v in pairs(p.sprite_model) do
		v.pos_scr = translate_to_view(v3d_add(p.pos,
			matrix_mul_add(m_player_rotation_x, matrix_mul_add(m_player_rotation_y, v.pos))))
...
function update_game_score(team, text)
	if no_control_timer <= 0 then
		...
		for _, v in pairs(players) do
			v.power = 0
			v.pos_data = v.team == serving_team and server_data[v.court_side + 1][v.team_member_index + 1] or
				receiver_data[v.court_side + 1][v.team_member_index + 1]
			v.mode = 2
			v.angle = v.pos_data.start_angle
...
function update_ball()
	if not ball.service then
		...
		ball.pos = v3d_add(service.pos,
			matrix_mul_add(matrix_rotate_x(0),
				matrix_mul_add(matrix_rotate_y( -service.angle), service.sprite_model[5].pos)))
	end
...
function init()
	...
	gfx.load_img(LAYER_SPRITESHEET, "tennis/spritesheet.png")
...
TILE_MAP = {
	-- upper
	199, 1, 24, 56, 160, 192, 7, 177, 0, 2, 13, 8, 128, 8, 32, 208, 136, 1, 2, 13, 8, 128, 24, 32, 208, 136, 2, 2, 13, 8,
	128, 40,
	32, 208, 136, 3, 2, 13, 8, 128, 56, 32, 208, 136, 4, 2, 13, 8, 128, 72, 32, 208, 136, 5, 2, 13, 8, 128, 88, 32, 208,
